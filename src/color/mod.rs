...
pub mod color {
    ...
    impl RGB {
        ...
        pub fn from_hex<S: ToString>(code : S) -> Result<RGB, HtmlColorConversionError> {
            ...
            if len != 7 { return Err(HtmlColorConversionError::InvalidStringLength); }
            if full_code.chars().nth(0).unwrap() != '#' { return Err(HtmlColorConversionError::MissingHash); }

            let red = format!("{}{}", full_code.chars().nth(1).unwrap(), full_code.chars().nth(2).unwrap() );
            let green = format!("{}{}", full_code.chars().nth(3).unwrap(), full_code.chars().nth(4).unwrap() );
            let blue = format!("{}{}", full_code.chars().nth(5).unwrap(), full_code.chars().nth(6).unwrap() );
...
mod tests {
    ...
    fn convert_red_to_hsv() {
        let red = color::RGB::from_f32(1.0, 0.0, 0.0);
        let hsv = red.to_hsv();
        assert_eq!(hsv.h, 0.0);
        assert_eq!(hsv.s, 1.0);
        assert_eq!(hsv.v, 1.0);
    ...
    fn convert_green_to_hsv() {
        let green = color::RGB::from_f32(0.0, 1.0, 0.0);
    ...
    fn convert_blue_to_hsv() {
        let blue = color::RGB::from_f32(0.0, 0.0, 1.0);
        let hsv = blue.to_hsv();
    ...
    fn convert_olive_to_hsv() {
        let grey = color::RGB::from_u8(128, 128, 0);
        let hsv = grey.to_hsv();
    ...
    fn convert_olive_to_rgb() {
        let grey = color::HSV::from_f32(60.0 / 360.0, 1.0, 0.5019608);
        let rgb = grey.to_rgb();
    ...
    // Tests that we make an HSV triplet at defaults and it is black.
    fn test_red_hex() {
        let rgb = color::RGB::from_hex("#FF0000").expect("Invalid hex string");
        assert_eq!(rgb.r, 1.0);
        assert_eq!(rgb.g, 0.0);
        assert_eq!(rgb.b, 0.0);
    }
    ...
    // Tests that we make an HSV triplet at defaults and it is black.
    fn test_green_hex() {
        let rgb = color::RGB::from_hex("#00FF00").expect("Invalid hex string");
        assert_eq!(rgb.r, 0.0);
        assert_eq!(rgb.g, 1.0);
        assert_eq!(rgb.b, 0.0);
    }
    ...
    // Tests that we make an HSV triplet at defaults and it is black.
    fn test_blue_hex() {
        let rgb = color::RGB::from_hex("#0000FF").expect("Invalid hex string");
        assert_eq!(rgb.r, 0.0);
        assert_eq!(rgb.g, 0.0);
        assert_eq!(rgb.b, 1.0);
    }
    ...
    // Tests that we make an HSV triplet at defaults and it is black.
    fn test_blue_named() {
        let rgb = color::RGB::named(color::BLUE);
        assert_eq!(rgb.r, 0.0);
        assert_eq!(rgb.g, 0.0);
        assert_eq!(rgb.b, 1.0);
    }
