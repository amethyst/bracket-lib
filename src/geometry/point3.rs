...
impl ops::Add<i32> for Point3 {
    type Output = Point3;
    fn add(self, rhs: i32) -> Point3 { Point3::new(self.x + rhs, self.y + rhs, self.z + rhs) }
}

/// Support subtracting a point from a point
impl ops::Sub<Point3> for Point3 {
    type Output = Point3;
    fn sub(self, rhs: Point3) -> Point3 { Point3::new(self.x - rhs.x, self.y - rhs.y, self.z - rhs.z) }
}

/// Support subtracting an int from a point
impl ops::Sub<i32> for Point3 {
    type Output = Point3;
    fn sub(self, rhs: i32) -> Point3 { Point3::new(self.x - rhs, self.y - rhs, self.z - rhs) }
}

/// Support multiplying a point by a point
impl ops::Mul<Point3> for Point3 {
    type Output = Point3;
    fn mul(self, rhs: Point3) -> Point3 { Point3::new(self.x * rhs.x, self.y * rhs.y, self.z * rhs.z) }
}

/// Support multiplying a point by an int
impl ops::Mul<i32> for Point3 {
    type Output = Point3;
    fn mul(self, rhs: i32) -> Point3 { Point3::new(self.x * rhs, self.y * rhs, self.z * rhs) }
}

/// Support multiplying a point by an f32
impl ops::Mul<f32> for Point3 {
    type Output = Point3;
    fn mul(self, rhs: f32) -> Point3 { Point3::new((self.x as f32 * rhs) as i32, (self.y as f32 * rhs) as i32, (self.z as f32 * rhs) as i32) }
}

/// Support dividing a point by a point
impl ops::Div<Point3> for Point3 {
    type Output = Point3;
    fn div(self, rhs: Point3) -> Point3 { Point3::new(self.x / rhs.x, self.y / rhs.y, self.z / rhs.z) }
}

/// Support dividing a point by an int
impl ops::Div<i32> for Point3 {
    type Output = Point3;
    fn div(self, rhs: i32) -> Point3 { Point3::new(self.x / rhs, self.y / rhs, self.z / rhs) }
}

/// Support dividing a point by an f32
impl ops::Div<f32> for Point3 {
    type Output = Point3;
    fn div(self, rhs: f32) -> Point3 { Point3::new((self.x as f32 / rhs) as i32, (self.y as f32 / rhs) as i32, (self.z as f32 / rhs) as i32) }
}
