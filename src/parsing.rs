...
impl DiceType {
    pub fn new(n_dice:i32, die_type:i32, bonus:i32) -> Self {
        DiceType{ n_dice, die_type, bonus }
    }
}

impl Default for DiceType {
    fn default() -> DiceType {
        DiceType{ n_dice: 1, die_type: 4, bonus: 0 }
    }
}
...
pub fn parse_dice_string(dice : &str) -> Result<DiceType, DiceParseError> {
    ...
    if !did_something { return Err(DiceParseError{}); }
    Ok(result)
}
...
mod tests {
    ...
    #[test]
    fn parse_1d20plus4() {
        assert_eq!( parse_dice_string("1d20+4").unwrap(), DiceType::new(1,20,4) );
    }
