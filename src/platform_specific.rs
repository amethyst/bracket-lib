...
RltkPlatform, PlatformGL, hal::WrappedContext, hal};
...
pub fn main_loop<GS: GameState>(mut rltk: Rltk, mut gamestate: GS) {
    ...
    render_loop.run(move |_running: &mut bool| {
        ...
        unsafe {
            rltk.key = hal::GLOBAL_KEY;
            rltk.mouse_pos = (hal::GLOBAL_MOUSE_POS.0, hal::GLOBAL_MOUSE_POS.1);
            rltk.left_click = hal::GLOBAL_LEFT_CLICK;
            rltk.shift = hal::GLOBAL_MODIFIERS.0;
            rltk.control = hal::GLOBAL_MODIFIERS.1;
            rltk.alt = hal::GLOBAL_MODIFIERS.2;
        }
        ...
        unsafe {
            hal::GLOBAL_KEY = None;
            hal::GLOBAL_MODIFIERS = (false, false, false);
            hal::GLOBAL_LEFT_CLICK = false;
        }
    });
}
