...
use std::sync::Mutex;
use crate::{RGB, Point, Rect, ColorPair, Rltk, Console};
...
pub enum DrawCommand { 
    ClearScreen,
    ClearToColor{color : RGB},
    SetTarget{console : usize},
    Set{pos: Point, color: ColorPair, glyph: u8},
    SetBackground{pos: Point, bg: RGB},
    Print{pos: Point, text: String},
    PrintColor{pos: Point, text: String, color: ColorPair},
    PrintCentered{y : i32, text: String},
    PrintColorCentered{y : i32, text: String, color: ColorPair},
    Box{pos: Rect, color: ColorPair},
    HollowBox{pos: Rect, color: ColorPair},
    DoubleBox{pos: Rect, color: ColorPair},
    HollowDoubleBox{pos: Rect, color: ColorPair},
    FillRegion{pos: Rect, color: ColorPair, glyph : u8},
    BarHorizontal{pos: Point, width: i32, n: i32, max: i32, color: ColorPair},
    BarVertical{pos: Point, height: i32, n: i32, max: i32, color: ColorPair}
}
...
impl DrawBatch {
    ...
    {
        self.batch.push(DrawCommand::Print{pos, text : text.to_string()});
        self
    }
    ...
    {
        self.batch.push(DrawCommand::PrintColor{pos, text : text.to_string(), color});
        self
    }
    ...
    {
        self.batch.push(DrawCommand::PrintCentered{y, text : text.to_string()});
        self
    }
    /// Prints text, centered to the whole console width, at vertical location y.
    pub fn print_color_centered<S: ToString>(&mut self, y: i32, text: S, color: ColorPair) -> &mut Self
    {
        self.batch.push(DrawCommand::PrintColorCentered{y, text : text.to_string(), color});
        self
    }
    ...
    {
        self.batch.push(DrawCommand::FillRegion{pos, color, glyph});
        self
    }

    /// Draw a horizontal progress bar
    pub fn bar_horizontal(&mut self, pos: Point, width: i32, n: i32, max: i32, color: ColorPair) -> &mut Self
    {
        self.batch.push(DrawCommand::BarHorizontal{pos, width, n, max, color});
        self
    }

    /// Draw a horizontal progress bar
    pub fn bar_vertical(&mut self, pos: Point, height: i32, n: i32, max: i32, color: ColorPair) -> &mut Self
    {
        self.batch.push(DrawCommand::BarVertical{pos, height, n, max, color});
        self
    }
}
...
pub fn render_draw_buffer(rltk : &mut Rltk) {
    ...
    buffer.iter().for_each(|cmd| {
        match cmd {
            ...
            DrawCommand::Set{pos, color, glyph} => rltk.set(pos.x, pos.y, color.fg, color.bg, *glyph),
            DrawCommand::SetBackground{pos, bg} => rltk.set_bg(pos.x, pos.y, *bg),
            DrawCommand::Print{pos, text} => rltk.print(pos.x, pos.y, &text),
            DrawCommand::PrintColor{pos, text, color} => rltk.print_color(pos.x, pos.y, color.fg, color.bg, &text),
            DrawCommand::PrintCentered{y, text} => rltk.print_centered(*y, &text),
            DrawCommand::PrintColorCentered{y, text, color} => rltk.print_color_centered(*y, color.fg, color.bg, &text),
            DrawCommand::Box{pos, color} => rltk.draw_box(pos.x1, pos.y1, pos.width(), pos.height(), color.fg, color.bg),
            DrawCommand::HollowBox{pos, color} => rltk.draw_hollow_box(pos.x1, pos.y1, pos.width(), pos.height(), color.fg, color.bg),
            DrawCommand::DoubleBox{pos, color} => rltk.draw_box_double(pos.x1, pos.y1, pos.width(), pos.height(), color.fg, color.bg),
            DrawCommand::HollowDoubleBox{pos, color} => rltk.draw_hollow_box_double(pos.x1, pos.y1, pos.width(), pos.height(), color.fg, color.bg),
            DrawCommand::FillRegion{pos, color, glyph} => rltk.fill_region(*pos, *glyph, color.fg, color.bg),
            DrawCommand::BarHorizontal{pos, width, n, max, color} => rltk.draw_bar_horizontal(pos.x, pos.y, *width, *n, *max, color.fg, color.bg),
            DrawCommand::BarVertical{pos, height, n, max, color} => rltk.draw_bar_vertical(pos.x, pos.y, *height, *n, *max, color.fg, color.bg),
        }
    });
    buffer.clear();
}