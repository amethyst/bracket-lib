...
// Utility functions
#[allow(dead_code)]
fn fast_floor(f:f32) -> i32 { if f >= 0.0 { f as i32 } else { f as i32 - 1 } }

#[allow(dead_code)]
fn fast_round(f:f32) -> i32 { if f >= 0.0 { (f + 0.5) as i32 } else { (f - 0.5) as i32 } }

#[allow(dead_code)]
fn fast_abs(i:i32) -> i32 { i32::abs(i) }

#[allow(dead_code)]
fn fast_abs_f(i:f32) -> f32 { f32::abs(i) }

#[allow(dead_code)]
fn lerp(a:f32, b:f32, t:f32) -> f32 { a + t * (b - a) }

#[allow(dead_code)]
fn interp_hermite_func(t:f32) -> f32 { t*t*(3. - 2. * t) }

#[allow(dead_code)]
fn interp_quintic_func(t:f32) -> f32 { t*t*t*(t*(t * 6. - 15.) + 10.) }
...
impl FastNoise {
    ...
    pub fn set_frequency(&mut self, frequency : f32) { self.frequency = frequency }
    pub fn get_frequency(&self) -> f32 { self.frequency }
    pub fn set_interp(&mut self, interp : Interp) { self.interp = interp }
    pub fn get_interp(&self) -> Interp { self.interp }
    pub fn set_noise_type(&mut self, nt : NoiseType) { self.noise_type = nt }
    pub fn get_noise_type(&self) -> NoiseType { self.noise_type }
    pub fn set_fractal_octaves(&mut self, octaves : i32) { self.octaves = octaves; self.calculate_fractal_bounding(); }
    pub fn get_fractal_octaves(&self) -> i32 { self.octaves }
    pub fn set_fractal_lacunarity(&mut self, lacunarity : f32) { self.lacunarity = lacunarity }
    pub fn get_fractal_lacunarity(&self) -> f32 { self.lacunarity }
    pub fn set_fractal_gain(&mut self, gain : f32) { self.gain = gain; self.calculate_fractal_bounding(); }
    pub fn get_fractal_gain(&self) -> f32 { self.gain }
    pub fn set_fractal_type(&mut self, fractal_type : FractalType) { self.fractal_type = fractal_type }
    pub fn get_fractal_type(&self) -> FractalType { self.fractal_type }
    pub fn set_cellular_distance_function(&mut self, cellular_distance_function : CellularDistanceFunction) { self.cellular_distance_function = cellular_distance_function }
    pub fn get_cellular_distance_function(&self) -> CellularDistanceFunction { self.cellular_distance_function }
    pub fn set_cellular_return_type(&mut self, cellular_return_type : CellularReturnType) { self.cellular_return_type = cellular_return_type }
    pub fn get_cellular_return_type(&self) -> CellularReturnType { self.cellular_return_type }
    pub fn get_cellular_distance_indices(&self) -> (i32, i32) { self.cellular_distance_index }
    pub fn set_cellular_jitter(&mut self, jitter : f32) { self.cellular_jitter = jitter }
    pub fn get_cellular_jitter(&self) -> f32 { self.cellular_jitter }
    pub fn set_gradient_perterb_amp(&mut self, gradient_perturb_amp : f32) { self.gradient_perturb_amp = gradient_perturb_amp }
    pub fn get_gradient_perterb_amp(&self) -> f32 { self.gradient_perturb_amp }
    ...
    pub fn set_cellular_distance_indices(&mut self, i1:i32, i2:i32) { 
        ...
        self.cellular_distance_index.0 = i32::min(i32::max(self.cellular_distance_index.0, 0), FN_CELLULAR_INDEX_MAX as i32);
        self.cellular_distance_index.1 = i32::min(i32::max(self.cellular_distance_index.1, 0), FN_CELLULAR_INDEX_MAX as i32);
    }
    ...
    pub fn index3d_12(&self, offset: u8, x:i32, y:i32, z:i32) -> u8 {
        self.perm12[(x as usize & 0xff) + self.perm[(y as usize & 0xff) + self.perm[(z as usize & 0xff) + offset as usize] as usize] as usize]
    }

    pub fn index4d_32(&self, offset: u8, x: i32, y: i32, z:i32, w: i32) -> u8 {
        self.perm[(x as usize & 0xff) + self.perm[(y as usize & 0xff) + self.perm[(z as usize & 0xff) + self.perm[(w as usize & 0xff) + offset as usize] as usize] as usize] as usize] & 31
    }
    ...
    pub fn index3d_256(&self, offset: u8, x:i32, y:i32, z:i32) -> u8 {
        self.perm[(x as usize & 0xff) + self.perm[(y as usize & 0xff) + self.perm[(z as usize & 0xff) + offset as usize] as usize] as usize]
    }

    pub fn index4d_256(&self, offset: u8, x: i32, y: i32, z:i32, w: i32) -> u8 {
        self.perm[(x as usize & 0xff) + self.perm[(y as usize & 0xff) + self.perm[(z as usize & 0xff) + self.perm[(w as usize & 0xff) + offset as usize] as usize] as usize] as usize]
    }
    ...
    fn val_coord_2d_fast(&self, offset: u8, x: i32, y: i32) -> f32 { VAL_LUT[self.index2d_256(offset, x, y) as usize] }
    fn val_coord_3d_fast(&self, offset: u8, x: i32, y: i32, z: i32) -> f32 { VAL_LUT[self.index3d_256(offset, x, y, z) as usize] }
    ...
    fn grad_coord_4d(&self, offset:u8, x: i32, y: i32, z: i32, w : i32, xd: f32, yd: f32, zd: f32, wd: f32) -> f32 {
        let lut_pos = self.index4d_32(offset, x, y, z, w) as usize;
        xd*GRAD_4D[lut_pos] + yd*GRAD_4D[lut_pos + 1] + zd*GRAD_4D[lut_pos + 2] + wd*GRAD_4D[lut_pos + 3]
    }
    ...
    fn get_white_noise4d(&self, x: f32, y: f32, z: f32, w: f32) -> f32 {
        unsafe {
            ...
            let result = self.val_coord_4d(self.seed as i32,
                xc ^ ( xc as i32 >> 16 ),
                yc ^ ( yc >> 16 ),
                zc ^ ( zc >> 16 ),
                wc ^ ( wc >> 16 )
    ...
    fn get_white_noise3d(&self, x: f32, y: f32, z: f32) ->f32 {
        unsafe {
            ...
            let result = self.val_coord_3d(self.seed as i32,
                xc ^ ( xc >> 16 ),
                yc ^ ( yc >> 16 ),
                zc ^ ( zc >> 16 )
            );
    ...
    fn get_white_noise(&self, x:f32, y:f32) -> f32 {
        unsafe {
            ...
            let result = self.val_coord_2d(self.seed as i32,
                xc ^ ( xc >> 16 ),
                yc ^ ( yc >> 16 )
            );
            result
    ...
    fn single_value_fractal_billow3d(&self, mut x : f32, mut y: f32, mut z: f32) -> f32 {
        ...
        while i < self.octaves {
            ...
            amp *= self.gain;
            sum += (fast_abs_f(self.single_value3d(self.perm[i as usize], x, y, z)) * 2.0 - 1.0) * amp;
    ...
    fn get_value3d(&self, x : f32, y: f32, z: f32) -> f32 {
        self.single_value3d(0, x * self.frequency, y * self.frequency, z * self.frequency)
    }
    ...
    fn single_value3d(&self, offset: u8, x: f32, y: f32, z: f32) -> f32 {
        ...
        let xf00 : f32 = lerp(self.val_coord_3d_fast(offset, x0, y0, z0), self.val_coord_3d_fast(offset, x1, y0, z0), xs);
        let xf10 : f32 = lerp(self.val_coord_3d_fast(offset, x0, y1, z0), self.val_coord_3d_fast(offset, x1, y1, z0), xs);
        let xf01 : f32 = lerp(self.val_coord_3d_fast(offset, x0, y0, z1), self.val_coord_3d_fast(offset, x1, y0, z1), xs);
        let xf11 : f32 = lerp(self.val_coord_3d_fast(offset, x0, y1, z1), self.val_coord_3d_fast(offset, x1, y1, z1), xs);
    ...
    fn single_value(&self, offset: u8, x : f32, y: f32) -> f32 {
        ...
        let xf0 = lerp(self.val_coord_2d_fast(offset, x0, y0), self.val_coord_2d_fast(offset, x1, y0), xs);
        let xf1 = lerp(self.val_coord_2d_fast(offset, x0, y1), self.val_coord_2d_fast(offset, x1, y1), xs);
    ...
    fn single_perlin_fractal_billow3d(&self, mut x : f32, mut y : f32, mut z : f32) -> f32 {
        ...
        while i < self.octaves {
            ...
            amp *= self.gain;
            sum += (fast_abs_f(self.single_perlin3d(self.perm[i as usize], x, y, z)) * 2.0 - 1.0) * amp;
            i += 1;
        }
    ...
    fn get_perlin3d(&self, x:f32, y:f32, z:f32) -> f32 {
        self.single_perlin3d(0, x*self.frequency, y*self.frequency, z*self.frequency)
    }
    ...
    fn single_perlin3d(&self, offset: u8, x : f32, y:f32, z:f32) -> f32 {
        ...
        let xf00 = lerp(self.grad_coord_3d(offset, x0, y0, z0, xd0, yd0, zd0), self.grad_coord_3d(offset, x1, y0, z0, xd1, yd0, zd0), xs);
        let xf10 = lerp(self.grad_coord_3d(offset, x0, y1, z0, xd0, yd1, zd0), self.grad_coord_3d(offset, x1, y1, z0, xd1, yd1, zd0), xs);
        let xf01 = lerp(self.grad_coord_3d(offset, x0, y0, z1, xd0, yd0, zd1), self.grad_coord_3d(offset, x1, y0, z1, xd1, yd0, zd1), xs);
        let xf11 = lerp(self.grad_coord_3d(offset, x0, y1, z1, xd0, yd1, zd1), self.grad_coord_3d(offset, x1, y1, z1, xd1, yd1, zd1), xs);
    ...
    fn single_perlin(&self, offset: u8, x : f32, y: f32) -> f32 {
        ...
        let xf0 = lerp(self.grad_coord_2d(offset, x0, y0, xd0, yd0), self.grad_coord_2d(offset, x1, y0, xd1, yd0), xs);
        let xf1 = lerp(self.grad_coord_2d(offset, x0, y1, xd0, yd1), self.grad_coord_2d(offset, x1, y1, xd1, yd1), xs);
    ...
    fn single_simplex_fractal_billow3d(&self, mut x : f32, mut y : f32, mut z : f32) -> f32 {
        ...
        while i < self.octaves {
            ...
            amp *= self.gain;
            sum += (fast_abs_f(self.single_simplex3d(self.perm[i as usize], x, y, z)) * 2.0 - 1.0) * amp;
            i += 1;
        }
    ...
    fn get_simplex3d(&self, x:f32, y:f32, z:f32) -> f32 {
        self.single_simplex3d(0, x * self.frequency, y * self.frequency, z * self.frequency)
    }
    ...
    fn single_simplex3d(&self, offset:u8, x:f32, y:f32, z:f32) -> f32 {
        ...
        {
            t *= t;
            n1 = t*t*self.grad_coord_3d(offset, i + i1 as i32, j + j1 as i32, k + k1 as i32, x1, y1, z1);
        }
        ...
        {
            t *= t;
            n2 = t*t*self.grad_coord_3d(offset, i + i2 as i32, j + j2 as i32, k + k2 as i32, x2, y2, z2);
        }
    ...
    fn get_simplex_4d(&self, x:f32, y:f32, z:f32, w:f32) -> f32 {
        self.single_simplex4d(0, x * self.frequency, y * self.frequency, z * self.frequency, w * self.frequency)
    }

    fn greater_1_0(&self, n:i32, greater_than:i32) -> i32 { if n >= greater_than { 1 } else { 0 } }
    ...
    fn single_simplex4d(&self, offset: u8, x : f32, y : f32, z: f32, w : f32) -> f32 {
        ...
        if t < 0.0 { n0 = 0.; }
            ...
            t *= t;
            n0 = t * t * self.grad_coord_4d(offset, i as i32, j as i32, k as i32, l as i32, x0, y0, z0, w0);
        }
        t = 0.6 - x1*x1 - y1*y1 - z1*z1 - w1*w1;
        if t < 0.0 { n1 = 0.; }
        else {
            t *= t;
            n1 = t * t * self.grad_coord_4d(offset, i as i32 + i1, j as i32 + j1, k as i32 + k1, l as i32 + l1, x1, y1, z1, w1);
        }
        t = 0.6 - x2*x2 - y2*y2 - z2*z2 - w2*w2;
        if t < 0.0 { n2 = 0.; }
        else {
            t *= t;
            n2 = t * t * self.grad_coord_4d(offset, i as i32 + i2, j as i32 + j2, k as i32 + k2, l as i32 + l2, x2, y2, z2, w2);
        }
        t = 0.6 - x3*x3 - y3*y3 - z3*z3 - w3*w3;
        if t < 0.0 { n3 = 0.; }
        else {
            t *= t;
            n3 = t * t * self.grad_coord_4d(offset, i as i32 + i3, j as i32 + j3, k as i32 + k3, l as i32 + l3, x3, y3, z3, w3);
        }
        t = 0.6 - x4*x4 - y4*y4 - z4*z4 - w4*w4;
        if t < 0.0 { n4 = 0.; }
        else {
            t *= t;
            n4 = t * t * self.grad_coord_4d(offset, i as i32 + 1, j as i32 + 1, k as i32 + 1, l as i32 + 1, x4, y4, z4, w4);
        }
    ...
    fn single_cubic_fractal_billow3d(&self, mut x:f32, mut y:f32, mut z:f32) -> f32 {
        ...
        while i < self.octaves {
            ...
            amp *= self.gain;
            sum += (fast_abs_f(self.single_cubic3d(self.perm[i as usize], x, y, z)) * 2.0 - 1.0) * amp;
            i += 1;
        }
    ...
    fn get_cubic3d(&self, x:f32, y:f32, z:f32) -> f32 {
        self.single_cubic3d(0, x * self.frequency, y * self.frequency, z * self.frequency)
    }
    ...
    fn single_cubic3d(&self, offset:u8, x:f32, y:f32, z:f32) -> f32 {
        ...
        return cubic_lerp(
            cubic_lerp(
                cubic_lerp(self.val_coord_3d_fast(offset, x0, y0, z0), self.val_coord_3d_fast(offset, x1, y0, z0), self.val_coord_3d_fast(offset, x2, y0, z0), self.val_coord_3d_fast(offset, x3, y0, z0), xs),
                cubic_lerp(self.val_coord_3d_fast(offset, x0, y1, z0), self.val_coord_3d_fast(offset, x1, y1, z0), self.val_coord_3d_fast(offset, x2, y1, z0), self.val_coord_3d_fast(offset, x3, y1, z0), xs),
                cubic_lerp(self.val_coord_3d_fast(offset, x0, y2, z0), self.val_coord_3d_fast(offset, x1, y2, z0), self.val_coord_3d_fast(offset, x2, y2, z0), self.val_coord_3d_fast(offset, x3, y2, z0), xs),
                cubic_lerp(self.val_coord_3d_fast(offset, x0, y3, z0), self.val_coord_3d_fast(offset, x1, y3, z0), self.val_coord_3d_fast(offset, x2, y3, z0), self.val_coord_3d_fast(offset, x3, y3, z0), xs),
                ys
            ),
            cubic_lerp(
                cubic_lerp(self.val_coord_3d_fast(offset, x0, y0, z1), self.val_coord_3d_fast(offset, x1, y0, z1), self.val_coord_3d_fast(offset, x2, y0, z1), self.val_coord_3d_fast(offset, x3, y0, z1), xs),
                cubic_lerp(self.val_coord_3d_fast(offset, x0, y1, z1), self.val_coord_3d_fast(offset, x1, y1, z1), self.val_coord_3d_fast(offset, x2, y1, z1), self.val_coord_3d_fast(offset, x3, y1, z1), xs),
                cubic_lerp(self.val_coord_3d_fast(offset, x0, y2, z1), self.val_coord_3d_fast(offset, x1, y2, z1), self.val_coord_3d_fast(offset, x2, y2, z1), self.val_coord_3d_fast(offset, x3, y2, z1), xs),
                cubic_lerp(self.val_coord_3d_fast(offset, x0, y3, z1), self.val_coord_3d_fast(offset, x1, y3, z1), self.val_coord_3d_fast(offset, x2, y3, z1), self.val_coord_3d_fast(offset, x3, y3, z1), xs),
                ys
            ),
            cubic_lerp(
                cubic_lerp(self.val_coord_3d_fast(offset, x0, y0, z2), self.val_coord_3d_fast(offset, x1, y0, z2), self.val_coord_3d_fast(offset, x2, y0, z2), self.val_coord_3d_fast(offset, x3, y0, z2), xs),
                cubic_lerp(self.val_coord_3d_fast(offset, x0, y1, z2), self.val_coord_3d_fast(offset, x1, y1, z2), self.val_coord_3d_fast(offset, x2, y1, z2), self.val_coord_3d_fast(offset, x3, y1, z2), xs),
                cubic_lerp(self.val_coord_3d_fast(offset, x0, y2, z2), self.val_coord_3d_fast(offset, x1, y2, z2), self.val_coord_3d_fast(offset, x2, y2, z2), self.val_coord_3d_fast(offset, x3, y2, z2), xs),
                cubic_lerp(self.val_coord_3d_fast(offset, x0, y3, z2), self.val_coord_3d_fast(offset, x1, y3, z2), self.val_coord_3d_fast(offset, x2, y3, z2), self.val_coord_3d_fast(offset, x3, y3, z2), xs),
                ys
            ),
            cubic_lerp(
                cubic_lerp(self.val_coord_3d_fast(offset, x0, y0, z3), self.val_coord_3d_fast(offset, x1, y0, z3), self.val_coord_3d_fast(offset, x2, y0, z3), self.val_coord_3d_fast(offset, x3, y0, z3), xs),
                cubic_lerp(self.val_coord_3d_fast(offset, x0, y1, z3), self.val_coord_3d_fast(offset, x1, y1, z3), self.val_coord_3d_fast(offset, x2, y1, z3), self.val_coord_3d_fast(offset, x3, y1, z3), xs),
                cubic_lerp(self.val_coord_3d_fast(offset, x0, y2, z3), self.val_coord_3d_fast(offset, x1, y2, z3), self.val_coord_3d_fast(offset, x2, y2, z3), self.val_coord_3d_fast(offset, x3, y2, z3), xs),
                cubic_lerp(self.val_coord_3d_fast(offset, x0, y3, z3), self.val_coord_3d_fast(offset, x1, y3, z3), self.val_coord_3d_fast(offset, x2, y3, z3), self.val_coord_3d_fast(offset, x3, y3, z3), xs),
                ys
            ),
    ...
    fn single_cubic(&self, offset:u8, x:f32, y:f32) -> f32 {
        ...
        return cubic_lerp(
            cubic_lerp(self.val_coord_2d_fast(offset, x0, y0), self.val_coord_2d_fast(offset, x1, y0), self.val_coord_2d_fast(offset, x2, y0), self.val_coord_2d_fast(offset, x3, y0), xs),
            cubic_lerp(self.val_coord_2d_fast(offset, x0, y1), self.val_coord_2d_fast(offset, x1, y1), self.val_coord_2d_fast(offset, x2, y1), self.val_coord_2d_fast(offset, x3, y1), xs),
            cubic_lerp(self.val_coord_2d_fast(offset, x0, y2), self.val_coord_2d_fast(offset, x1, y2), self.val_coord_2d_fast(offset, x2, y2), self.val_coord_2d_fast(offset, x3, y2), xs),
            cubic_lerp(self.val_coord_2d_fast(offset, x0, y3), self.val_coord_2d_fast(offset, x1, y3), self.val_coord_2d_fast(offset, x2, y3), self.val_coord_2d_fast(offset, x3, y3), xs),
                ...
                for xi in xr-1 .. xr + 2
                {
                    for yi in yr-1 .. yr + 2
                    {
                        for zi in zr-1 .. zr+2
                        {
                            ...
                            let vecX = xi as f32 - x + CELL_3D_X[lutPos as usize] * self.cellular_jitter;
                            let vecY = yi as f32 - y + CELL_3D_Y[lutPos as usize] * self.cellular_jitter;
                            let vecZ = zi as f32 - z + CELL_3D_Z[lutPos as usize] * self.cellular_jitter;
                ...
                for xi in xr-1 .. xr+2
                {
                    for yi in yr-1 .. yr+2
                    {
                        for zi in zr - 1 .. zr + 2
                        {
                            ...
                            let newDistance = fast_abs_f(vecX) + fast_abs_f(vecY) + fast_abs_f(vecZ);
                ...
                for xi in xr-1 .. xr+2
                {
                    for yi in yr - 1 .. yr+2
                    {
                        for zi in zr-1 .. zr+2
                        {
                            ...
                            let newDistance = (fast_abs_f(vecX) + fast_abs_f(vecY) + fast_abs_f(vecZ)) + (vecX * vecX + vecY * vecY + vecZ * vecZ);
    ...
    fn single_cellular_2edge3d(&self, x:f32, y:f32, z:f32) -> f32 {
                    ...
                    for yi in yr-1 .. yr+2
                    {
                        for zi in zr-1 .. zr+2
                        {
                            ...
                            for i in (0 .. self.cellular_distance_index.1).rev() {
                                distance[i as usize] = f32::max(f32::min(distance[i as usize], newDistance), distance[i as usize - 1]);
                            }
        ...
        match self.cellular_return_type {
            ...
            CellularReturnType::Distance2Add => { distance[self.cellular_distance_index.1 as usize] + distance[self.cellular_distance_index.0 as usize] }
            CellularReturnType::Distance2Sub => { distance[self.cellular_distance_index.1 as usize] - distance[self.cellular_distance_index.0 as usize] }
            CellularReturnType::Distance2Mul => { distance[self.cellular_distance_index.1 as usize] * distance[self.cellular_distance_index.0 as usize] }
            CellularReturnType::Distance2Div => { distance[self.cellular_distance_index.0 as usize] / distance[self.cellular_distance_index.1 as usize] }
            _ => { 0.0 }
        }
    }
            ...
            CellularDistanceFunction::Euclidean => {
                for xi in xr-1 .. xr+2
                {
                    for yi in yr-1 .. yr+2
                    {
                        ...
                        let vecX = xi as f32 - x + CELL_2D_X[lutPos as usize] * self.cellular_jitter;
                        let vecY = yi as f32 - y + CELL_2D_Y[lutPos as usize] * self.cellular_jitter;
            ...
            CellularDistanceFunction::Natural => {
                for xi in xr-1 .. xr+2
                {
                    for yi in yr-1 .. yr+2
                    {
                        ...
                        let newDistance = (fast_abs_f(vecX) + fast_abs_f(vecY)) + (vecX * vecX + vecY * vecY);
    ...
    fn single_gradient_perturb3d(&self, offset: u8, warpAmp : f32, frequency: f32, x:&mut f32, y:&mut f32, z:&mut f32) {
        let xf = *x * frequency;
        let yf = *y * frequency;
        let zf = *z * frequency;
    ...
    fn single_gradient_perturb(&self, offset:u8, warpAmp:f32, frequency:f32, x:&mut f32, y:&mut f32) {
        let xf = *x * frequency;
        let yf = *y * frequency;
