...
use bevy::{
    ...
    prelude::{CoreStage, Plugin, StageLabel, SystemStage},
    utils::HashMap,
};
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, StageLabel)]
pub enum BracketStage {
    BracketTermUpdate,
    BracketTermDiagnostics,
}
    ...
    fn build(&self, app: &mut bevy::prelude::App) {
        ...
        if self.with_diagnostics {
            app.add_stage_before(
                CoreStage::Update,
                BracketStage::BracketTermDiagnostics,
                SystemStage::single_threaded(),
            );
            app.add_system_to_stage(BracketStage::BracketTermDiagnostics, update_timing);
            app.add_system_to_stage(BracketStage::BracketTermDiagnostics, update_mouse_position);
        }

        app.add_stage_after(
            CoreStage::Update,
            BracketStage::BracketTermUpdate,
            SystemStage::single_threaded(),
        );

        if self.auto_apply_batches {
            app.add_system_to_stage(BracketStage::BracketTermUpdate, apply_all_batches);
        }

        app.add_system_to_stage(BracketStage::BracketTermUpdate, update_consoles);
        app.add_system_to_stage(BracketStage::BracketTermUpdate, replace_meshes);
        app.add_system_to_stage(BracketStage::BracketTermUpdate, window_resize);
        app.add_system_to_stage(BracketStage::BracketTermUpdate, fix_images);
