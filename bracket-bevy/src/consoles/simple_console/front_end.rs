...
impl ConsoleFrontEnd for SimpleConsole {
    ...
    fn print(&mut self, mut x: usize, y: usize, text: &str) {
        ...
        for glyph in bytes {
            let idx = self.at(x, y);
            self.terminal[idx] = TerminalGlyph{ glyph, foreground: Color::WHITE.as_rgba_f32(), background: Color::BLACK.as_rgba_f32() };
            x += 1;
        }
    }

    fn print_color(&mut self, mut x: usize, y: usize, text: &str, foreground: Color, background: Color) {
        let bytes = string_to_cp437(text);
        for glyph in bytes {
            let idx = self.at(x, y);
            self.terminal[idx] = TerminalGlyph{ glyph, foreground: foreground.as_rgba_f32(), background: background.as_rgba_f32() };
            x += 1;
        }
    }
