use bevy::{prelude::{Handle, Mesh, Color, Assets, Commands}, sprite::ColorMaterial};
use crate::{cp437::string_to_cp437, fonts::FontStore};
use self::back_end::{SimpleConsoleBackend, SimpleBackend};
...
impl SimpleConsole {
    ...
    pub(crate) fn initialize(&mut self, fonts: &[FontStore], meshes: &mut Assets<Mesh>) {
        let mut back_end = SimpleBackend::new(
    ...
    pub(crate) fn spawn(&self, commands: &mut Commands, material: Handle<ColorMaterial>, idx: usize) {
...
impl ConsoleFrontEnd for SimpleConsole {
    ...
    fn print_color(&mut self, mut x: usize, y: usize, text: &str, foreground: Color) {
        let bytes = string_to_cp437(text);
        for glyph in bytes {
            let idx = self.at(x, y);
